...
public class KitChgCommand implements ISubCommand {
	...
	public boolean onSubCommand(Player sender, Command cmd, List<String> args) {
		...
		if(sender.hasPermission("kit.change")) {
			...
			if(!(KitsUtils.getKit(args.get(1)) == null)) {
				...
				//EXISTING KIT ITEM
				Kit k = KitsUtils.getKit(args.get(1));
				for(Item i : k.getItemList()) {
					int z = 0;
				
					invKit.setItem(z, new ItemStack(Material.getMaterial(i.getId()), i.getQuantity(), (short) 0, (byte) i.getMeta()));
					z++;
					if(z == 7 || z == 16) {
						z = z + 2;
					}
